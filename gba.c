...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
    for (int i = row; i < row + height; i++) {
        ...
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[(WIDTH * i) + col];
...
void drawFullScreenImageDMA(const u16 *image) {
   ...
   DMA[DMA_CHANNEL_3].cnt = (WIDTH * HEIGHT) | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT | DMA_ON;
}
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
    for (int i = row; i < row + height; i++) {
        DMA[DMA_CHANNEL_3].src = &image[(WIDTH * i) + col];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[(WIDTH * i) + col];
...
void fillScreenDMA(volatile u16 color) {
    DMA[DMA_CHANNEL_3].src = &color;
    DMA[DMA_CHANNEL_3].dst = videoBuffer;
    DMA[DMA_CHANNEL_3].cnt = (WIDTH * HEIGHT) | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED | DMA_ON;
}
